...
O mapa de posição do padeiro de malha calcula o local da geometria de malha de alto polímero e salva em uma textura. É semelhante ao padeiro de posição base, mas pode produzir resultados mais precisos.
...
| *Parâmetro* | *Descrição* |
| --- | --- |
| **Modo** | Controla quais informações serão computadas na textura da posição.Valores possíveis:<ul data-preserve-html="true"><li data-preserve-html="true"><strong>Todos os eixos:</strong> transforma a posição dos eixos X, Y e Z nos canais de RGB da textura de saída.</li><li data-preserve-html="true"><strong>Um eixo:</strong> transforma um único eixo na textura de saída como uma imagem em tons de cinza.</li></ul> |
