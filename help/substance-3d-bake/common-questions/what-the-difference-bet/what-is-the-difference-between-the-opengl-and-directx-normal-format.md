...
> **Explicação**
> 
> OpenGL e DirectX são duas APIs gráficas (conjuntos de funções) que os programadores usam em seu aplicativo para dialogar com a GPU (Unidade de processamento gráfico). Em termos de mapas normais, a diferença resulta em como o canal verde de uma textura deve ser interpretado. O OpenGL espera que o primeiro pixel esteja na parte inferior, enquanto o DirectX espera que ele esteja na parte superior. Muitas vezes, é por isso que, em várias discussões técnicas, é recomendável tentar inverter o canal verde de uma mapa normal para ver se ela se comporta melhor à medida que inverte os valores de pixel (o primeiro se torna o último). O OpenGL pode ser referido como **Y+** (de baixo para cima), enquanto o DirectX é referido como **Y-** (de cima para baixo).
> 
> Para saber qual formato usar, consulte o aplicativo de destino no qual suas texturas serão usadas.
