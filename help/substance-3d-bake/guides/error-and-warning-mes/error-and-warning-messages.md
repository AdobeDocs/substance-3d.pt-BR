...
Abaixo está a lista de todas as mensagens de erro que podem aparecer ao fazer bake com o software Substance.

## Qualquer Baker

| *Mensagem* | *Descrição* |
| --- | --- |
| Baker não disponível. | Essa mensagem de erro geralmente é seguida por mensagens de erro adicionais, geralmente relacionadas a problemas de GPU. Isso pode acontecer se a GPU for muito antiga e não atender aos [requisitos técnicos](https://www.allegorithmic.com/products/tech-specs) do software. |
| O conjunto UV [X] não existe. | O Baker tentou trabalhar com um determinado conjunto UV que não está presente na malha de baixo polígono. |
| Não é possível carregar a cena da URL. | Essa mensagem significa que o baker não foi capaz de carregar o arquivo de malha, geralmente a malha alta de poli. Algumas razões podem ser a origem desta mensagem:<ul data-preserve-html="true"><li data-preserve-html="true">O arquivo de malha referenciado não existe mais.</li><li data-preserve-html="true">O arquivo de malha está corrompido ou danificado e não pode ser lido.</li><li data-preserve-html="true">No momento, a malha está sendo editada por outro aplicativo e não pode ser lida.</li></ul> |
...
| A cena não tem UVs. Cancelando faço bake. | Se nenhuma malha na cena tiver UVs, o processo de fça bake será cancelado. |
...
| A malha [nome da malha] não tem UVs para uv definido [X]. | O Baker tentou trabalhar com um determinado conjunto UV que não está presente na malha de baixo polígono. |

## Qualquer Baker “Da malha”
...
| *Mensagem* | *Descrição* |
| --- | --- |
| Falha ao carregar textura detalhada. | Não foi possível carregar a textura definida nas configurações do baker. Pode ser porque o arquivo está realmente ausente no disco ou porque está corrompido e não pode ser lido. |
