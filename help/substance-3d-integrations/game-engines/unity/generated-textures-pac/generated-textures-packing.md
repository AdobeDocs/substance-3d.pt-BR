...
breadcrumb-title: ''
description: Entenda como o Substance gera texturas no Unity e configure a embalagem de textura para entradas ideais de sombreador.
...
As Texturas geradas mostram as saídas da Substance que são computadas pela Substance Engine para criar texturas. Essas texturas são alimentadas nas entradas do sombreador. Por padrão, apenas as entradas base usadas pelo sombreador são criadas. Se a opção “Gerar todas as saídas” estiver ativada, todas as texturas serão mostradas aqui.
...
1. O botão sRGB funciona de forma semelhante à opção sRGB (textura de cor) nas Configurações de importação de textura. Permite definir se uma textura deve ser interpretada em espaço gama (sRGB) ou linear. O plug-in Substance lida com essa interpretação automaticamente, mas pode ser substituído se necessário.
...
É possível empacotar uma textura no canal alfa de outra textura usando o menu suspenso. Cada textura gerada tem um menu suspenso que contém uma lista de todas as saídas de textura geradas pelos materiais Substance. Basta escolher um mapa na lista para embalá-lo no canal alfa da textura. A opção Origem é o canal alfa da textura.

Nesta imagem, selecionei o mapa de heights:

![](../../../assets/screen-shot-2022-03-29-at-2-48-33-pm.png)

Na imagem a seguir, você pode ver que a saída do height está sendo compactada no canal alfa do mapa de cores base.

![](https://helpx-prod.scene7.com/is/image/HelpxProd/screen-shot-2022-03-29-at-2-53-20-pm-copy?$png$&jpegSize=200&wid=1248)

## Mapeamento de textura de saída
