...
| --- | --- | --- |
| **float** público *GetInputFloat*(**cadeia** inputName) | Obter Entrada De Substance **Flutuante** | **Cadeia** *inputName* Nome da entrada no SBSAR |
| público **int** *SetInputFloat*(**cadeia** inputName, valor **float**) | Atualizar entrada do Substance **Flutuante** | **Cadeia** i *nputName* Nome da entrada no **Flutuante** *valor* SBSAR usado para atualizar o parâmetro |
...
| **void Substance.Game.SubstanceGraph**.*SetInputString*(string inputName, valor de string) | Atualizar entrada da sequência de caracteres do Substance | **Cadeia de caracteres** *inputName* Nome da entrada no **valor** {6 *do SBSAR usado para atualizar o parâmetro* |
...
| **void Substance.Game.SubstanceGraph**.*SetInputTexture*(string inputName, valor Texture2D) | Atualizar entrada Substance Texture2D | **Cadeia de caracteres** *inputName* Nome da entrada no **valor *SBSAR Texture2D*** usado para atualizar o parâmetro |
| **Texture2D Substance.Game.SubstanceGraph**.*GetInputTexture*( string inputName) | Obter Entrada Substance Texture2D | **Cadeia** *inputName* Nome da entrada no SBSAR |
| **Substance VetorInt.Game.SubstanceGraph**.*GetTexturesResolution*() | Obtenha a resolução das texturas das Configurações de destino do gráfico (os valores de Vetor4Int x = width, y = height, podem ser 32, 64, 128, 256, 512, 1024, 2048 e 4096) | Nenh. |
| **int Substance.Game.SubstanceGraph**.*SetTexturesResolution*(tamanho de Vetor2Int) | Defina a resolução das texturas das Configurações de destino do gráfico (Vetor2Int’s x = width, y = height, os valores podem ser 32, 64, 128, 256, 512, 1024, 2048 e 4096) Retorna 0 se bem-sucedido; caso contrário: -1. | **Vetor2Int** *size* usado para atualizar o parâmetro**.** |
| **Listar Substance.Game.SubstanceGraph**.*GetGeneratedTextures*() | Retorna todos os objetos Substance Texture2D usados pelo sombreador de material do gráfico. | Nenh. |
| **int Substance.Game.SubstanceGraph**.*Bake*(textura Texture2D, caminho absoluto da cadeia de caracteres) | Gere arquivos .png para todos os objetos Substance Texture2D usados pelo sombreador de material do gráfico. | Nenh. |
