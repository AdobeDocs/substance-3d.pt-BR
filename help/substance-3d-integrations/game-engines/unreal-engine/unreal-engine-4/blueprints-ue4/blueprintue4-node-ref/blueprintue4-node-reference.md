---
helpx_url: "https://helpx.adobe.com/br/substance-3d-integrations/game-engines/unreal-engine/unreal-engine-4/blueprints-ue4/blueprintue4-node-reference.html"
breadcrumb-title: ''
description: Guia de referência para todos os nós de Substance Blueprint disponíveis no Unreal Engine 4 para operações de materiais.
...
helpx_learn_topic: ""
helpx_tags: ""
title: Referência do Nó Blueprint(UE4)
...
# Blueprint(UE4): referência de nó

## Nós de Substance gerais:

| Nome | Entradas | Descrição |
| --- | --- | --- |
| **GetSubstances** | Material | Retorna uma matriz de Instâncias de Gráfico do Substance usadas por um material. Se você criar um material que usa saídas de textura de duas instâncias de gráfico diferentes, essa função retornará essas duas instâncias de gráfico. |
| **GetSubstanceTextures** | **SubstanceGraphInstance** | Retorna uma matriz de todas as texturas ativadas e calculadas atualmente do parâmetro de entrada Instância de Gráfico do Substance. |
| **GetGraphName** | **SubstanceGraphInstance** | Retorna o nome do gráfico conforme definido no Designer. |
| **GetFactoryName** | **SubstanceGraphInstance** | Retorna o nome da **GraphInstanceFactory** que foi usada para criar a **SubstanceGraphInstance** passada para este nó. |
...
| **DefinirEntradaInt** | Entrada: **Identificador** (Cadeia de Caracteres)Entrada: **InputValues** (matriz int) | Altere o valor de uma entrada encontrada pelo Identificador. De dentro de um jogo, é necessário renderizar a substância usando **AyncRender** ou **SyncRender** para que as alterações sejam aplicadas. |
...
| **GetInputFloat** | Identificador (String) | Retorna uma matriz de flutuações com os valores atuais de um parâmetro de entrada. |
